...
| 5 | **Auth** — login at `/{locale}`, set `access_token`, proxy guard for private routes, logout, unauthorized / access-denied pages | Done |
| 6 | **Panel shell** — `(panel)` group; nav + top bar + main; `/home`; menu; projects; profile/settings/search/upgrade/lang | Done |
...
**Phase 7 — App features** (pick the next feature to build)
